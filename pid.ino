...
void command_corrections() {
  ...
  if (adjusted_throttle < 1100) reset_controller();

  // Shift tilt compensation to angle mode
  if (adjusted_throttle > 1200) {

    float max_compensation = 1800.0f / adjusted_throttle;
    float tilt_compensation = 1.0f / (cosf(roll_angle * DEG_TO_RAD) * cosf(pitch_angle * DEG_TO_RAD));

    if (tilt_compensation > max_compensation) tilt_compensation = max_compensation;
    adjusted_throttle *= tilt_compensation;
  }
